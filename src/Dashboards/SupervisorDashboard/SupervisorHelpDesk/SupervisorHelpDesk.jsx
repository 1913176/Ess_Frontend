...
import { GetSupervisorTicketList } from "@/api/ServerAction";
  ...
  const TicketForm = ({ isOpen, onClose, selectedService, services, onSuccess }) => {
    ...
    const fetchRecipients = async () => {
      ...
      try {
        ...
        const mappedRecipients = response.data
          .map((recipient) => ({
            value: recipient[idField] || `unknown-${Math.random().toString(36).substr(2, 9)}`,
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        setError('Error submitting ticket: ' + (error.response?.data?.detail || error.message));
      }
    };
...
function SupervisorHelpDeskComponent() {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const tickets = await GetSupervisorTicketList();
      ...
      const mappedTickets = tickets.map((ticket) => ({
        ...
        employee_name: ticket.supervisor_name || "N/A",
